...
This manual is inconsistent as to how this is notated (most likely because of many cooks in the kitchen). The scaling factor is $q$, but there are multiple notations for it. Some contributors use $q$ = $n$. Others prefer a shorthand $@n$ notation.

However it is notated, the scaling factor represent's the \emph{programmer's} division of the 31 bits of the LGP-21's word into a \emph{logical} integer portion and fractional portion. The hardware binary point remains fixed, but the programmer's may shift. 
...
\subsubsection*{Arithmetic Operations and Scale Factors}
Addition and subtraction require that the $q$ factors of both operands be identical before the operation is executed. The LGP-21 treats every 31-bit word as a raw fixed-point fraction with the hardware binary point just to the right of the sign bit. If you add two numbers with different $q$ factors without shifting them so their $q$ factors match, the machine will simply perform the operation requested on the two numbers.
    ...
    \end{tabular}
    \end{center}
The meaning of this number is ambiguous (and categorically invalid). Depending on which scale factor you \emph{think} the result is using, your answer is now either $1.125$ (if you assume the result is $q=1$ or $9.0$ (if you assume $q=4$. The machine will not throw an error, trigger an overflow, or warn you; it will simply calculate perfectly represented garbage because the binary point in the two numbers is in \emph{two different places}.

Scale factor changes occur during multiplication and division as well: multiplication \emph{adds} the scale factors, and division \emph{subtracts} them. After a multiplication or division, the result must be shifted left or right to ensure that the result conforms to the scale factor desired.

Multiplying $q=4$ value by a $q=5$ value yields a $q=9$ value. This can be left-shifted to restore it to a $q=4$ or $q=5$value; you should note that this many bits of precision have been lost in the operation. Not perfect, but acceptable.

However, division, as mentioned previously \emph{requires} that the divisor have a smaller $q$ than the dividend. If it is larger, the operation overflows, and the most significant figures are lost in the operation. Dividing a $q=9$ number by a $q=2$ one will yield a result that is a $q=7$ number. On the other hand, dividing a $q=1$ number by an $q=9$ one results in a scale factor of $-8$, an overflow, and a loss of the 8 most significant figures of the result. To return this (invalid) result to a $q=1$ value, it would need to be shifted right \emph{8} bits -- and those 8 bits are now zero and the actual value has been lost. If a $q=9$ value was desired, the overflow will have lost \emph{17} bits. 
...
\hline
\textbf{Location} & \textbf{Instruction} & \textbf{Address} & \textbf{Description} \\ \hline
$\alpha-1$        & \texttt{B}           & \texttt{N}       & Bring word 1 of floating-point number $N$. \\ \hline
$\alpha$          & \texttt{R}           & \texttt{L+offset}& Store the return address ($\alpha+2$) inside the subroutine. \\ \hline
$\alpha+1$        & \texttt{U}           & \texttt{L}       & Unconditional branch to the subroutine entry. \\ \hline
\end{tabularx}
\end{table}
...
\begin{enumerate}
	...
	\item Now the subroutine has the address of the first of the three words making up the floating-point number, and can proceed.
\end{enumerate}
