...
Multiplication and division also change the scale factor. Multiplication \emph{adds} $q$ values, division \emph{subtracts} them: a $q=4$ value times a $q=5$ value gives a $q=9$ result. The bit width of the word doesn't change, so the further the result's $q$ drifts from where you want it, the more significant figures you've lost.

Division has the extra restriction noted earlier: the divisor's magnitude must exceed the dividend's. Equivalently, if subtracting $q_\text{divisor}$ from $q_\text{dividend}$ gives a negative result, the operation overflows and the high-order bits are gone for good. Dividing a $q=1$ value by a $q=9$ value formally yields $q=-8$ --- i.e.\ the top 8 bits of the answer were thrown away.
