...
\subsubsection*{INPUT}
The input variable for \texttt{B1-10.0} is an angle in degrees, $\theta$, for which the function value is to be calculated. $\theta$ must be in the accumulator $@9$ when the jump to the subroutine occurs. The angle is reduced to an angle between 0° and 360° by the subroutine before evaluation.
...
\midrule
$\alpha-1$      & \texttt{B}            & $\theta$       & Load $\theta$ at $@9$ into the accumulator.\\
...
\subsubsection*{INPUT}
The input variable for \texttt{B1-10.1} is an angle $\theta$, whose function value is to be calculated. $\theta$ must be in the accumulator $@3$ and have a value $\le 7.9999999$ when the call to the subroutine occurs. The subroutine reduces the angle to an equivalent angle $\beta$ with an absolute value <= $1.75$.
...
\textbf{Memory Location} & \textbf{Instruction} & \textbf{Address} & \textbf{Meaning} \\
\midrule
$\alpha-1$      & \texttt{B}            & $\theta$       & Load $\theta$ at $@3$ into the accumulator.\\
...
\subsubsection*{INPUT}
The input variable for \texttt{B1-11.0} is the value $N$ at $@9$ whose arctangent is to be computed. This value must be loaded into the accumulator before the subroutine is called.
...
\textbf{Memory Location} & \textbf{Instruction} & \textbf{Address} & \textbf{Meaning} \\
\midrule
$\alpha-1$      & \texttt{B}            & $N$       & Load $N$ at $@9$ into the accumulator.\\
...
In the default configuration (at $@9$), the maximum value the machine can physically represent is $2^9-1$ or 511.999999. As an angle approaches 90º, its tangent approaches infinity. Since the arctangent is the inverse function, this means that your application cannot use the constants at $@9$ if the input values need to be larger than 511; calculating with the constants at $@9$ will overflow.
...
\paragraph{Warnings}
The subroutine does not validate that the $q$ value used for the patchable constants and your input argument match! If you patch the internal constants at $\alpha+56$ and $\alpha+59$ to use a custom scale factor (say, at $@12$), you must scale your input argument to that exact same scale factor before calling the routine. Cross-scaled parameters and constants will silently yield erroneous results and possible overflow conditions.
...
\subsubsection*{INPUT}
The value $N$ at $@1$, with $-1 \le N \le 1$, must be in the accumulator when the function is called.
...
\textbf{Memory Location} & \textbf{Instruction} & \textbf{Address} & \textbf{Meaning} \\
\midrule
$\alpha-1$      & \texttt{B} & $N$                  & Load $N$ at $@1$ into the accumulator.\\
...
\label{subsub:halfQ}
There's a lot hiding inside that throwaway ``the $q$ value of the result is half of the argument's $q$ value''. You may also be wondering why the $q$ value of the argument \emph{must} be even, especially since we're doing all this math at $@1$ and $@9$.
...
\end{equation}
If your scale factor $q$ is an odd number (like the standard at $@1$ or the at $@9$ used elsewhere in these library functions), then $q/2$ is a fraction (4.5)! You cannot shift a binary word by 4.5 bits, meaning that you can't make the result of the square root operation commensurate to any $q$.

Forcing the programmer to provide an input with an even $q$ (like at $@2$, at $@8$, or at $@18$) means that the resulting scale factor after the square root function finishes will be an integer (at $@1$, at $@4$, or at $@9$, respectively) and that the result will be valid. 
