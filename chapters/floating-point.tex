...
\section*{RUNNING THE INTEPRETER}
...
\subsubsection*{Termination Stopcodes}
...
\nopagebreak
\medskip
\noindent$^{\dagger}$ The stopcode (\texttt{'}) immediately following the initial address ends the address and starts number entry for the input group.

\medskip
\noindent$^{\ddagger}$ A single stopcode with no numeric input terminates the input group.

\medskip
\noindent$^{\P}$ Positive quantities are encoded with \emph{no} prefix; negative values should have an explicit minus sign (\texttt{-}). Do not punch a plus sign (\texttt{+}) for positive values.

\medskip
\noindent$^{\S}$ This single stopcode ends the second data group.

\medskip
\noindent$^{\parallel}$ This stopcode following the group terminator stopcode tells the interpreter to exit input mode and resume execution.

\medskip

\noindent Reading the data in Table \ref{data-input-table} results in the following data in memory. Group 1 is read and the floating-point numbers are built\footnote{Translator's note: I've preserved the original manual's representation here; suffice it to say that the values indicated are handwaving the actual values in memory.} starting at location \texttt{5500}:
\noindent\texttt{5500} \texttt{-33000.0} \\
...
\vspace{1em}
\noindent The processing loop for the second data group is terminated by a single stop-code just as Group 1 was. Note the \emph{second} stopcode, which serves to signal the interpreter that input is complete and execution may continue.
\medskip
\noindent\textbf{Special case:} Please note that any number meant to be input as a literal value of zero \emph{must} be explicitly encoded and punched onto the tape as \texttt{0'} rather than as just a stopcode (\texttt{'})\footnote{This is different from other input routines, particularly the \ref{J1-10.0} loader.}. Explicitly entering the zero values is necessary to allow the input routine to differentiate a true numerical zero from a structural group-termination/input-termination command.

\subsection*{6. Data Output}
...
\medskip
\noindent This physical teleprinter serialization format represents the standard mathematical expression:
\begin{equation*}
...
\medskip
\noindent Note how the leading column remains perfectly uniform; the decimal points align vertically on the page regardless of whether the value carries a negative sign or an implicit positive blank space.

\subsubsection*{Output Limits and Constraints}
...
\subsection*{7. FUNCTION SUBROUTINES}

\subsubsection*{SINE-COSINE}
...
\subsubsection*{LOGARITHM AND EXPONENTIAL FUNCTION}
...
\subsubsection*{Running the interpreter}
...
\begin{tabular}{llll}
$\alpha$             & R                    & $L_0$            &; Store starting address of floating-point instructions. \\
$\alpha+1$           & U                    & $L_0$            &; Entry into the interpreter system. \\
$\alpha+2$           & \multicolumn{2}{l}{\dots}               &; Floating-point instructions. \\
$\alpha+3$           & \multicolumn{2}{l}{\dots}               &; \\
$\alpha+n$           & E                    & 0000             &; Exit from the interpreter system.\\
$\alpha+n+1$         & \multicolumn{2}{l}{\dots}               &; Resume fixed-point instructions. \\
\end{tabular} 
...
\noindent\textbf{Scratch Storage} --- The program uses cells 6300 to 6363 as working storage.
...
\midrule
$L_0+0947$ & \parbox[t]{5cm}{Negative argument for square root} & \parbox[t]{4cm}{Press \textsf{START}. The psuedo-instruction exits with the original argument in the accumulator.} \\
\midrule
$L_0+0312$ & \parbox[t]{5cm}{Exponent 333 for Hold or Clear} & \parbox[t]{4cm}{Press \textsf{START}. Hold or Clear pseudo-instructions are bypassed.} \\
\midrule
$L_0+0710$ & \parbox[t]{5cm}{Division by zero or non-floated number} & Execution cannot continue. \\
\midrule
$L_0+1808$ & \parbox[t]{5cm}{Exponent > 99 during data input} & \parbox[t]{4cm}{Press \textsf{START}. The Flexowriter tabs to the next stop without printing, and the code exits to the core interpreter.} \\
\midrule
$L_0+2100$ & \parbox[t]{5cm}{Argument for $sin$/$cos \ge 2^26$} & \parbox[t]{4cm}{Press \textsf{START}. The \texttt{S0000} or \texttt{C0000} operation is skipped.} \\
\midrule
$L_0+2426$ & \parbox[t]{5cm}{Argument for $\log_e X \le 0$} & \parbox[t]{4cm}{Press \textsf{START}. The pseudo-instruction returns zero and control returns to the main program.} \\
\midrule
$L_0+2629$ & \parbox[t]{5cm}{Argument for $e^X \ge 128$} & \parbox[t]{4cm}{Press \textsf{START}. The pseudo-instruction is skipped.} \\
\bottomrule
\end{tabular}
\end{center}

\subsubsection*{Program Control Switches}
...
\item It's good practice to load the entirety of H1-11.1, and to then punch out the individual sub-libraries by means of J4-10.2 or J4-10.3. This enables you to load only the parts that you require using J1-10.0 or by means of J5-10.0. This allows you to verify checksums on load.
...
\noindent
\subsubsection{Coding sheet for Horner's Method example}
...
\begin{tabular}{|p{0.18\textwidth}|c|p{0.10\textwidth}|p{0.18\textwidth}|c|p{0.18\textwidth}|p{0.28\textwidth}|}
	\hline
...
\subsection*{Data input sheet for Horner's Method input}
\begin{center}
\small
\ttfamily
...
\subsection*{Why do we have \emph{two} floating point packages?}
...
\begin{tabular}{llll}
$\alpha$             & R                    & $L_0$            &\texttt{; Start of interpreter} \\
$\alpha+1$           & U                    & $L_0$            &\texttt{; Entry into the interpreter system} \\
$\alpha+2$           & I                    & 0000			   &\texttt{; read a, b, c, d as float} \\
$\alpha+2$           & B                    & \texttt{a}        &\texttt{; Load a} \\
$\alpha+3$           & A                    & \texttt{b}        &\texttt{; Add b} \\
$\alpha+4$           & H                    & \texttt{t1}     &\texttt{; Store result in t1} \\
$\alpha+5$           & B                    & \texttt{c}        &\texttt{; Load c}  \\
$\alpha+6$           & A                    & \texttt{d}        &\texttt{; Add d} \\
$\alpha+7$           & H                    & \texttt{t2}     &\texttt{; Store result in t2} \\
$\alpha+8$           & B                    & \texttt{t1}        &\texttt{; Load t1} \\
$\alpha+9$           & A                    & \texttt{t2}        &\texttt{; Multiply t2} \\
$\alpha+10$          & P                    & 0000             &\texttt{; Print result} \\
$\alpha+11$          & E                    & 0000             &\texttt{; Exit from the interpreter}\\
$\alpha+n+1$         & \multicolumn{2}{l}{\dots}               &\texttt{; Resume fixed-point} \\                 
\end{tabular}
...
\subsubsection*{Coding example for Interpreter 2}
...
\subsubsection*{Reading an LGP-21 coding sheet}
...
\subsubsection*{How the Horner's Method coding sheet and data input sheet would have been used}
