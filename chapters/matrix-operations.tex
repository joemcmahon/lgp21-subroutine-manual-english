...
\subsection{Matrix Transposition 1}
...
\subsection{Complex Operations Interpretive System}
...
The following table outlines the maximum execution durations required for each individual complex operation, measured in drum rotations and milliseconds.

\medskip
\noindent
\begin{tabular}{crr|crr}
\toprule
\textbf{Instruction} & \textbf{Drum Rotations} & \textbf{Time (ms)} & \textbf{Instruction} & \textbf{Drum Rotations} & \textbf{Time (ms)} \\
\midrule
\texttt{B}           & 11                      & 561                & \texttt{P}           & 17                      & 1167\footnote{Notice how left-shifting is significantly slower than right-shifting. On a magnetic disk memory system like the LGP-21, bit-shifting requires track-recirculation delays and masking. Left shifts require repeated additions or multiplication loops to slide data up toward the sign bit, adding extra revolutions, whereas right shifts could take advantage of standard hardware truncation.}               \\
...
\begin{tabular}{lp{6.5cm}l}
    ...
    \midrule
    \texttt{L+122}    & Negative Instruction encountered (Illegal format). & Fatal error. Do not force resume. \\
    \texttt{L+135}    & Invalid \texttt{N-} Type instruction variation.    & Fatal error. Do not force resume. \\
    \texttt{L+154}    & Invalid \texttt{T-} Type instruction variation.    & Fatal error. Do not force resume. \\
    \bottomrule
\end{tabular}
