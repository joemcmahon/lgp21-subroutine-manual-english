...
\subsubsection*{PURPOSE}
\texttt{H1-10.0} is a fixed-point interpreter that lets you write code with complex numbers ($a+bi$) as if they were ordinary reals. It also provides left and right bit-shifting, address-register arithmetic, and a loop-end comparison --- all without leaving the interpreter loop.
...
\subsubsection*{INPUT}
A complex number occupies two consecutive locations at the same $q$ factor: the real part at \texttt{ttss} and the imaginary part at \texttt{ttss+1}.

Because complex values are two words wide, the interpreter keeps its working registers in memory rather than in the hardware accumulator: a pseudo-accumulator at \texttt{L+33}, an address register at \texttt{L+219}, and a saved copy of the hardware accumulator at \texttt{L+59} (where \texttt{L} is the interpreter's base address).

The interpreter has 15 instructions, listed below. For arithmetic instructions, the address field \texttt{ttss} points at the real component and the interpreter operates on both halves automatically. For other instructions, \texttt{ttss} is a single-word address. Only the instructions listed below are valid inside an interpretive sequence.

\paragraph{1. Arithmetic Instructions}
\begin{description}[leftmargin=1.5em, itemsep=4pt]
    \item[\texttt{Bttss} (Bring):] Load \texttt{ttss}/\texttt{ttss+1} into the pseudo-accumulator.
    \item[\texttt{Attss} (Add):] Add \texttt{ttss}/\texttt{ttss+1} to the pseudo-accumulator.
    \item[\texttt{Sttss} (Subtract):] Subtract \texttt{ttss}/\texttt{ttss+1} from the pseudo-accumulator.
    \item[\texttt{Mttss} (Multiply):] Multiply the pseudo-accumulator by \texttt{ttss}/\texttt{ttss+1}.\footnote{\textbf{Translator's note:} A complex multiplication $(a+bi)(c+di)$ requires computing $(ac-bd) + (ad+bc)i$. Performing this in a single interpretive step saves a massive amount of main-program setup and temporary storage management.}
    \item[\texttt{Dttss} (Divide):] Divide the pseudo-accumulator by \texttt{ttss}/\texttt{ttss+1}.
    \item[\texttt{Httss} (Hold):] Store the pseudo-accumulator into \texttt{ttss}/\texttt{ttss+1} (the pseudo-accumulator is unchanged).
    \item[\texttt{Cttss} (Clear):] Store the pseudo-accumulator into \texttt{ttss}/\texttt{ttss+1}, then zero the pseudo-accumulator.
\end{description}

\paragraph{2. Auxiliary Instructions}
\begin{description}[leftmargin=1.5em, itemsep=4pt]
    \item[\texttt{XRO0nn} (Shift Right):] Shift both halves of the pseudo-accumulator right by \texttt{nn} bits ($0 \le \texttt{nn} \le 10$).
    \item[\texttt{XPO0nn} (Shift Left):] Shift both halves of the pseudo-accumulator left by \texttt{nn} bits ($0 \le \texttt{nn} \le 10$).
    \item[\texttt{Uttss} (Unconditional Jump):] Jump to the pseudo-instruction at \texttt{ttss}. Can't be used to exit the interpreter --- \texttt{ttss} must point at another interpreter instruction.
    \item[\texttt{XE0000} (Exit):] Leave the interpreter. The next word is executed as a native hardware instruction.
\end{description}

\paragraph{3. Address Modification Instructions}
\begin{description}[leftmargin=1.5em, itemsep=4pt]
    \item[\texttt{Ettss} (Set Address Register):] Load the address field of the word at \texttt{ttss} into the address register.
    \item[\texttt{Ittss} (Increment Address):] Add the value \texttt{ttss} to the address register.
    \item[\texttt{Yttss} (Inject Address):] Write the address register's value into the address field of the word at \texttt{ttss}.
    \item[\texttt{Zttss} (Test and Skip):] If \texttt{ttss} equals the address register, skip the next pseudo-instruction; otherwise continue with the next instruction.
\end{description}

\subsubsection*{CALLING SEQUENCE}
The calling sequence enters the interpreter using the standard \texttt{R}/\texttt{U} pair; \texttt{L} is the interpreter's base address. The \texttt{R} at $\alpha$ stashes the address of the first pseudo-instruction ($\alpha+2$) inside the interpreter so it knows where to start fetching, and the \texttt{U} at $\alpha+1$ branches into the interpreter. An \texttt{XE} somewhere in the pseudo-instruction sequence exits back to native execution.
...
\subsubsection*{REMARKS}
The first instruction at $\alpha+2$ must not be an \texttt{XE0000} --- the interpreter would exit immediately before doing anything useful.

\subsubsection*{TRANSLATOR NOTES}
The address-arithmetic pseudoinstructions (\texttt{E}, \texttt{I}, \texttt{Y}, \texttt{Z}) make this more than just a complex-math library --- they let a program walk through multi-element arrays of complex numbers without ever leaving the interpreter loop.
\newpage
