...
\subsubsection*{PURPOSE}
This program uses the Floating-Point Interpretive System 1 (Section~\ref{H1-11.0}). \texttt{D1-12.0} multiplies a matrix by a column vector and stores the resulting product vector in a designated set of memory locations. 
...
\begin{enumerate}
	...
	\item the starting address for the destination product vector. Upon return to the main program, the calculated product vector resides in the specified locations.
\end{enumerate}
...
\begin{enumerate}[label=\arabic*., leftmargin=1.5em, itemsep=2pt]
    ...
    \item The elements of a column vector stored in consecutive locations. The number of elements in the vector must equal the number of columns ($j$) in the matrix.
\end{enumerate}
\noindent
...
\subsubsection*{OUTPUT}
The individual elements of the calculated product vector are saved sequentially in consecutive locations, beginning at address $P$.
...
The input variables are:
\begin{enumerate}[label=\arabic*., leftmargin=1.5em, itemsep=2pt]
    \item The elements of matrix $A$ ($n \times m$), stored row-by-row in continuous sequential locations.
    \item The elements of matrix $B$ ($m \times p$), stored row-by-row in continuous sequential locations. The number of rows in matrix $B$ must precisely equal the number of columns in matrix $A$.
\end{enumerate}
\noindent
...
The data must conform to the floating-point layout required by Floating-Point Interpreter 1 (Section~\ref{H1-11.0}). Dimensions must be less than 64, and total memory overhead requires a block of $(n \times m) + (m \times p) + (n \times p)$ locations to accommodate the inputs and the trailing results workspace.
...
\subsubsection*{OUTPUT}
The result of ``Matrix Multiplication 1'' is the product matrix $C$, structured with $n$ rows and $p$ columns, stored row-by-row in continuous sequential locations starting at location $C$. Element $c_{21}$ immediately follows element $c_{1p}$, and so forth.
...
\end{enumerate}
\noindent
Upon return to the main program, the resulting matrix resides in the designated locations.
...
\subsubsection*{OUTPUT}
The individual elements of the resulting matrix $C_{ij}$ are saved in floating-point format in consecutive locations, beginning at address $C$.
...
\subsubsection*{PURPOSE}
\texttt{D1-15.0} transposes a square matrix stored in memory and saves the transposed results into designated locations. The input consists of the elements of a square matrix whose rank is strictly less than 32. Both fixed-point and floating-point data formats are supported. The total number of elements must not exceed the available memory space.
