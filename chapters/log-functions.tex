...
\subsubsection*{Translator's Notes}
The behavior on an invalid argument ($N \le 0$) is a little odd. Halting at \texttt{L+8} leaves execution close to the error, fine; but why also leave $N - (1\text{ at }@30)$ in the accumulator instead of just $N$?

``$1$ at $@30$'' means a single \texttt{1} bit in position 30 of the word, i.e.\ the value $2^{-30}$ --- one bit short of the LSB. Subtracting it from $N$ is essentially $N - \epsilon$: the result looks almost identical to $N$ if you read it as a number, but the low end of the word will differ.
