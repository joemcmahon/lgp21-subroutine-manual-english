...
\subsubsection*{TRANSLATOR'S NOTE}
The calling sequence for this particular subroutine may look extremely odd for those of us who are familiar with the more common sequence, which stores the return address directly into the subroutine's \texttt{U} instruction that returns control to the caller. We store the return address into an instruction, and then \emph{immediately jump to it}? How is that even going to work?
