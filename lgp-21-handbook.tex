...
The \texttt{B} loads $\theta$ into the accumulator; any instruction getting the appropriate theta into the accumulator is valid here. The \texttt{R} saves the return address at the subroutine's return \texttt{U} instruction, and the \texttt{U} to $\alpha$ (the start of the subroutine) calls it. Execution of the main program resumes just after the \texttt{U} to $\alpha$.
...
\texttt{B1-10.0} calculates the sine or cosine of an angle given in degrees. If the given angle is greater than 360°, the subroutine reduces it to the corresponding equivalent between 0 and 360° before calculating the function value. A 9th-degree polynomial is used for approximation. \texttt{B1-10.0} uses the standard LGP-21 calling sequence. The angle argument must supplied in the accumulator$@9$. The function value, including its sign, will be in the accumulator$@1$ when the subroutine returns.
...
\end{tabular}
\end{center}
The calling sequence for cosine is exactly the same as for sine, except the subroutine is entered at four instructions past the origin address of the \texttt{B1-10.0} subroutine.
...
This is a function call; argument is loaded into the accumulator before the call, and the result is returned in the accumulator.
...
\end{align}
Solutions are obtained in all four quadrants. The values $x$ and $y$ must be given at the same $q$. The result represents a partial result that can be converted into degrees or degrees of arc. \texttt{B1-11.1} is a standard LGP-21 subroutine, requiring an \texttt{R} instruction to modify the return address in the subroutine's terminal \texttt{U} instruction. When calling the subroutine, $y$ must have been stored in the subroutine's designated parameter storage, and $x$ must be in the accumulator. Upon returning to the main program, the result will be in the accumulator.
...
The Decimal Fill Keyword (Start fill) causes the input program to store decimal words in ascending order into consecutive cells, beginning with memory location \texttt{TTSS}.
...
If incorrect information is read, the program selects the typewriter, prints "err", and halts. See under "Error Stops".\ref{subsec:PIRerrors}

[translator notes: Fail-Safe Hardware Routing: The fact that \texttt{J1-10.1} automatically reverts control back to the primary console ("standard typewriter") upon entry at $0000$ or after an error stop is a fail-safe. If an external high-speed paper tape reader or secondary device feeds malformed data or fails mid-transfer, the system won't lock up or isolate the operator; it immediately brings the main console printer back online so the engineer can see the error state and intervene.

Physical Carriage Limits: Data tapes should contain a carriage return (CR) after every four to eight words simply because the mechanical realities of working with a hardware Flexowriter as console. If a continuous stream of data was read off a tape without embedded carriage returns, the physical typing element would reach the end of its travel at the right margin and continute to push against the margin as more characters are printed. This both imposes mechanical strain and
overprints the input as an unreadable blur of characters at the edge of the log sheet.]
