...
\begin{center}
\begin{tabular}{lll}
    \texttt{B} & \texttt{parameter} & \text{; Load theta@9 into the accumulator} \\
    \texttt{R} & \texttt{$\alpha+n$} & \text{; Store return address at end of sub} \\
    \texttt{U} & \texttt{$\alpha$} & \text{; call the sine routine} \\
\end{tabular}
\end{center}

On the LGP-21, there is no such thing as a stack or recursion. All subroutine returns must be done by modifying a \textbf{U} (unconditional jump) instruction in the subroutine itself to set the return address. Because of this self-modifying code, it is not possible to have multiple calls to a subroutine without destroying any previous return address.

Programmers should take care to ensure that they carefully manage the hierarchy of subroutine calls to prevent a second call to a subroutine, directly or indirectly, before it returns.

Subroutine addresses in the original text are referred to as $\alpha$ or $\alpha+n$ because the code in question is relocatable; I have retained this usage, but programmers needing to use these functions should substitute the base address that they have relocated the subroutine to as the value of $\alpha$.

\subsection{Repeated/redundant information}

Because this is a reference manual meant for use by people who were casually programmers and principally something else entirely, this manual consistently repeats information regarding the setup of subroutine calls and how they work. I have condensed this where possible; some subroutines use unusual parameterization (such as patching the values of words in the subroutine itself!) and I have retained all of the setup information as it was originally written in these cases.

I have otherwise condensed these repetitions as "standard LPG-21 calling sequence" where appropriate as noted in \ref{subsec:standard-call}.
...
\subsubsection*{Fractional fixed-point on the LGP-21}
\label{sec:fractional-fixed-point}
The LGP-21 treats all 31-bit words strictly as fractions ranging from $-1$ to $+1$, with the physical binary point fixed immediately to the right of the sign bit. To work with integers or mixed numbers, the programmer must mentally "shift" this binary point to the right by $n$ bits (as indicated by the $@n$ notation). 

For example, a value at $@3$ allocates 3 bits for the integer portion of a number (allowing values up to $7.999999$) and the remaining 27 bits for the fraction. As noted above, the programmer is \emph{entirely responsible} for tracking this scale factor across operations, and must manually shift, multiply, or divide intermediate values as needed to maintain the desired scale factor.
...
\subsection*{Addition/Subtraction Pitfalls}
 Addition and subtraction require that the scale factors ($@n$) of both operands be identical before the operation is executed. The LGP-21 treats every 31-bit word as a raw fixed-point fraction. If you add two numbers with different $q$ factors without shifting them appropriately, the machine will blindly align their bits and perform the operation, smashing incommensurate units of completely different mathematical weights into the same columns. 
    ...
    It is vital to understand that the LGP-21 hardware itself does not know about scale factors and has no way to check them or track them.  Scale factor occur during multiplication and division; multiplication \emph{adds} the scale factors, and division \emph{subtracts} them. After a multiplication or division, the result must be shifted left or right to ensure that the result conforms to the scale factor desired.

    As an example, if an @1 value were to be divided by an @9 one, the resulting scale factor would be $-8$, meaning that to return it to an $@1$ value, the result would need to be shifted right \emph{8} bits, and for an $@9$ result, it would need to be shifted right \emph{17} bits.
...
The document is organized in broad sections of related code, broadly broken into mathematical functions, a pair of floating-point calculators, input and output support code, utilities, and a grab-bag of sorts, searches, data maintenance/backup, and programming utilities. (The utility that automates self-modifying code loops is particularly engaging!)

The original documents is a reference and really just has each program described in order with no further organization. The programs were logically broken up into sections by the codes (e.g., \textbf{B1-10.0}), but the sections were not named or described. I've taken the liberty of adding sections to match the codes, with each lettered prefix denoting a new section. This means that some sections contain a fair number of one or two-page descriptions, while others (notably the floating-point calculators) contain only one or two quite extensive ones.
...
This work \emph{is} a translation, and I cannot guarantee I have been completely accurate. I've done my best to be sure my interpretation of what's documented here is correct, but I have not run or tested all of the programs referenced, and I can't make any guarantees as to their fitness and accuracy, nor can I be sure that the document I've translated here is in fact accurate itself!

This document will be up on GitHub; please feel free to submit corrections and errata there if you find anything that needs a fix. I intend to license this document such that it can be freely built upon; my only request is that you credit me for the basic work if you create a derivative work from this one.
